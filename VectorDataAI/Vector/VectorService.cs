...
using OpenAI;
using VectorDataAI;
using VectorDataAI.Models;

namespace VectorDataAI.Vector{
    public class VectorService
    {
        // Add properties and methods as needed
        List<CloudService> cloudServices = new();
        ...
        public async Task SearchServiceByPrompt(string prompt = "Which Azure service should I use to store my secret?")
        {
            IConfigurationRoot config = new ConfigurationBuilder().AddUserSecrets<Program>().Build();
            string? model = config["EmbeddingModelName"]; //text-embedding-3-small
            ...
            ReadOnlyMemory<float> queryEmbedding = await generator.GenerateVectorAsync(prompt);
